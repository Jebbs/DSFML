...
/**
 * $(U Event) holds all the informations about a system event that just
 * happened. Events are retrieved using the `Window.pollEvent` and
 * `Window.waitEvent` functions.
 *
 * An $(U Event) instance contains the type of the event (mouse moved, key
 ...
 * filled; all other* members will have undefined values and must not be read if
 * the type of the event doesn't match. For example, if you received a
 * `KeyPressed` event, then you must read the `event.key` member, all other
 * members such as event.MouseMove or `event.text` will have undefined values.
 *
 * Example:
 * ---
 * Event event;
